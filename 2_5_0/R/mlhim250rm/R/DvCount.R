...
DvCount <- function(pcm) {
  ...
  if (length(pcm) > 0)
  {
    ...
    n <- XML::getNodeSet(pcm, '//dvcount-value', nsDEF)
    if (length(n) > 0)
    {
      dvcount_value <- XML::xmlValue(n[[1]])
    }

    n <- XML::getNodeSet(pcm, '//dvcount-units/dvstring-value', nsDEF)
    if (length(n) > 0)
    {
      units_value <- XML::xmlValue(n[[1]])
    }

    n <- XML::getNodeSet(pcm, '//dvcount-units/label', nsDEF)
    if (length(n) > 0)
    {
    ...
    data <- data.frame(label, vtb, vte,
                       reference_ranges, normal_status, dvcount_value,  units_value, units_label, magnitude_status, error, accuracy,
