...
#'
#'
#' Returns a data.frame of the collected nodes of a DvRatio restriction from the XML fragment.
#'
#' The vectors are:
#'
...
#'
#' dvratio_value - Decimal value of the measurement.
#'
...
DvRatio <- function(pcm) {
  ...
  if (length(pcm) > 0)
  {
    ...
    n <- XML::getNodeSet(pcm, '//dvratio-value', nsDEF)
    if (length(n) > 0)
    {
      dvratio_value <- XML::xmlValue(n[[1]])
    }

    n <- XML::getNodeSet(pcm, '//dvratio-units/dvstring-value', nsDEF)
    if (length(n) > 0)
    {
      units_value <- XML::xmlValue(n[[1]])
    }

    n <- XML::getNodeSet(pcm, '//dvratio-units/label', nsDEF)
    if (length(n) > 0)
    {
