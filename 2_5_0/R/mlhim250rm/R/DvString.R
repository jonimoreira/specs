...
#'
#' MLHIM 2.5.0 DvString
#'
#'
#' Returns a data.frame of the collected nodes of a DvString restriction from the XML fragment.
#'
#' The vectors are:
#'
...
#'
#' dvstring_value - The string value of the item.
#'
#' dvstring_language - Optional indicator of the localised language in which this data-type is written.
#'                     The value space of language is the set of all strings that are valid language identifiers as defined in RFC-3066.
#'
#'
...
DvString <- function(pcm) {
  ...
  if (length(pcm) > 0)
  {
    ...
    # PCMdefaults
    dvstring_language <- NA
    dvstring_language <- NA

    n <- XML::getNodeSet(pcm, '//dvstring-value', nsDEF)
    if (length(n) > 0)
    {
      dvstring_value <- XML::xmlValue(n[[1]])
    }

    n <- XML::getNodeSet(pcm, '//dvstring-language', nsDEF)
    if (length(n) > 0)
    {
      dvstring_language <- XML::xmlValue(n[[1]])
    }

    data <- data.frame(label, vtb, vte,
                       dvstring_value, dvstring_language,
