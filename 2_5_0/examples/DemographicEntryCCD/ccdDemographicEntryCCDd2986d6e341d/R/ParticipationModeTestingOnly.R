...
#' The XML element name is pcs-661a8db4-172b-48a3-b609-b06297d6a708 as a restriction of the DvString
...
parseParticipationModeTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-661a8db4-172b-48a3-b609-b06297d6a708', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-661a8db4-172b-48a3-b609-b06297d6a708'
      data$ccd <- XML::xmlName(root)
