...
#' \code{Additional Address Line(s) (US)  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Additional Address Line(s) (US)  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-7cf3d0f7-e206-406f-aa3c-ea5032b9c409 as a restriction of the DvString
...
#' AdditionalAddressLinesUSTestingOnly <- getAdditionalAddressLinesUSTestingOnly(files) 
#' 
#' @export
getAdditionalAddressLinesUSTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseAdditionalAddressLinesUSTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseAdditionalAddressLinesUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-7cf3d0f7-e206-406f-aa3c-ea5032b9c409', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-7cf3d0f7-e206-406f-aa3c-ea5032b9c409'
      data$ccd <- XML::xmlName(root)
