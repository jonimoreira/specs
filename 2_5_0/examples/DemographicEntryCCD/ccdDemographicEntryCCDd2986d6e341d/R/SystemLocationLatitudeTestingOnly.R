...
#' \code{System Location Latitude  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{System Location Latitude  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-b945f2c8-b645-4d41-8694-e6aa593f208f as a restriction of the DvString
...
#' SystemLocationLatitudeTestingOnly <- getSystemLocationLatitudeTestingOnly(files) 
#' 
#' @export
getSystemLocationLatitudeTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseSystemLocationLatitudeTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseSystemLocationLatitudeTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-b945f2c8-b645-4d41-8694-e6aa593f208f', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-b945f2c8-b645-4d41-8694-e6aa593f208f'
      data$ccd <- XML::xmlName(root)
