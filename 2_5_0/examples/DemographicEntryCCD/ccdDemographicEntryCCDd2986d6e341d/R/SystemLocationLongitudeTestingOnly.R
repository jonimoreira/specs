...
#' The XML element name is pcs-ccac1c5e-f7db-42c6-8b46-e7b0f403d61b as a restriction of the DvString
...
parseSystemLocationLongitudeTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-ccac1c5e-f7db-42c6-8b46-e7b0f403d61b', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-ccac1c5e-f7db-42c6-8b46-e7b0f403d61b'
      data$ccd <- XML::xmlName(root)
