...
#' The XML element name is pcs-ede50e62-ea86-4d69-aeb6-a5fc12355ce3 as a restriction of the DvBoolean
...
parseTestDvBooleanTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-ede50e62-ea86-4d69-aeb6-a5fc12355ce3', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvBoolean)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-ede50e62-ea86-4d69-aeb6-a5fc12355ce3'
      data$ccd <- XML::xmlName(root)
