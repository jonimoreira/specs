...
#' The XML element name is pcs-8c917c63-9bfd-4913-96e6-11678968d308 as a restriction of the DvString
...
parseBloodPressureDeviceTestingUseOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-8c917c63-9bfd-4913-96e6-11678968d308', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-8c917c63-9bfd-4913-96e6-11678968d308'
      data$ccd <- XML::xmlName(root)
