...
#' The XML element name is pcs-2cd70ea9-6d12-4fbc-af7c-ed7e90ba9d07 as a restriction of the DvFile
...
parseAttestationViewTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-2cd70ea9-6d12-4fbc-af7c-ed7e90ba9d07', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvFile)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-2cd70ea9-6d12-4fbc-af7c-ed7e90ba9d07'
      data$ccd <- XML::xmlName(root)
