...
#' The XML element name is pcs-634143df-bff2-42b5-84d6-a20040fcda22 as a restriction of the DvLink
...
parseConnectedToTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-634143df-bff2-42b5-84d6-a20040fcda22', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvLink)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-634143df-bff2-42b5-84d6-a20040fcda22'
      data$ccd <- XML::xmlName(root)
