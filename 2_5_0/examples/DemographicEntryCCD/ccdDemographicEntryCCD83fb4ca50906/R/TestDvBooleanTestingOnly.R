...
#' \code{Test DvBoolean  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Test DvBoolean  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-fc20baeb-cde1-4e48-a435-8cbdd0744c05 as a restriction of the DvBoolean
...
#' TestDvBooleanTestingOnly <- getTestDvBooleanTestingOnly(files) 
#' 
#' @export
getTestDvBooleanTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseTestDvBooleanTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseTestDvBooleanTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-fc20baeb-cde1-4e48-a435-8cbdd0744c05', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvBoolean)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-fc20baeb-cde1-4e48-a435-8cbdd0744c05'
      data$ccd <- XML::xmlName(root)
