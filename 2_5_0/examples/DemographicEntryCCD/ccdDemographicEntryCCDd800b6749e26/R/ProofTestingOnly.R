...
#' \code{Proof  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Proof  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-09b1a7df-c810-4a83-a7d5-235d600533fd as a restriction of the DvFile
...
#' ProofTestingOnly <- getProofTestingOnly(files) 
#' 
#' @export
getProofTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseProofTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseProofTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-09b1a7df-c810-4a83-a7d5-235d600533fd', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvFile)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-09b1a7df-c810-4a83-a7d5-235d600533fd'
      data$ccd <- XML::xmlName(root)
