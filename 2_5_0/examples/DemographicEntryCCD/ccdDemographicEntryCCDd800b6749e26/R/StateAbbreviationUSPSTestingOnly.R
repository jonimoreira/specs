...
#' \code{State Abbreviation (USPS)  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{State Abbreviation (USPS)  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-6fb84cfa-a865-473e-ae89-cc829c1bcf01 as a restriction of the DvString
...
#' StateAbbreviationUSPSTestingOnly <- getStateAbbreviationUSPSTestingOnly(files) 
#' 
#' @export
getStateAbbreviationUSPSTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseStateAbbreviationUSPSTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseStateAbbreviationUSPSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-6fb84cfa-a865-473e-ae89-cc829c1bcf01', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-6fb84cfa-a865-473e-ae89-cc829c1bcf01'
      data$ccd <- XML::xmlName(root)
