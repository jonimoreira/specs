...
#' \code{Participation Mode - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Participation Mode - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-b8841007-acb8-4e81-90c8-594c8d825c7e as a restriction of the DvString
...
#' ParticipationModeTestingOnly <- getParticipationModeTestingOnly(files) 
#' 
#' @export
getParticipationModeTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseParticipationModeTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseParticipationModeTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-b8841007-acb8-4e81-90c8-594c8d825c7e', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-b8841007-acb8-4e81-90c8-594c8d825c7e'
      data$ccd <- XML::xmlName(root)
