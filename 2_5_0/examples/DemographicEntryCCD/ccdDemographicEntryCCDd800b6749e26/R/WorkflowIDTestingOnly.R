...
#' The XML element name is pcs-18f4858d-f2df-4e06-bf1f-b3885331dce3 as a restriction of the DvLink
...
parseWorkflowIDTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-18f4858d-f2df-4e06-bf1f-b3885331dce3', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvLink)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-18f4858d-f2df-4e06-bf1f-b3885331dce3'
      data$ccd <- XML::xmlName(root)
