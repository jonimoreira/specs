...
#' \code{Latitude (Decimal Format)  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Latitude (Decimal Format)  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-e6fb31c0-61b9-4c31-a7a8-3f15582e33aa as a restriction of the DvString
...
#' LatitudeDecimalFormatTestingOnly <- getLatitudeDecimalFormatTestingOnly(files) 
#' 
#' @export
getLatitudeDecimalFormatTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseLatitudeDecimalFormatTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseLatitudeDecimalFormatTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-e6fb31c0-61b9-4c31-a7a8-3f15582e33aa', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-e6fb31c0-61b9-4c31-a7a8-3f15582e33aa'
      data$ccd <- XML::xmlName(root)
