...
#' The XML element name is pcs-3cd4a457-f08c-428d-9db6-b4ecee063751 as a restriction of the DvString
...
parseParticipationFunctionTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-3cd4a457-f08c-428d-9db6-b4ecee063751', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-3cd4a457-f08c-428d-9db6-b4ecee063751'
      data$ccd <- XML::xmlName(root)
