...
#' \code{System ID  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{System ID  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-27527140-08e6-4d65-b102-240f5339f47a as a restriction of the DvString
#' The vectors are: label, vtb, vte, #' dvstring_value, dvstring_language, #' ccd, sourceName,
#' 
#' A reusable system ID for Audit
#' 
#' @references
...
#' SystemIDTestingOnly <- getSystemIDTestingOnly(files) 
#' 
#' @export
getSystemIDTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseSystemIDTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseSystemIDTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-27527140-08e6-4d65-b102-240f5339f47a', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-27527140-08e6-4d65-b102-240f5339f47a'
      data$ccd <- XML::xmlName(root)
