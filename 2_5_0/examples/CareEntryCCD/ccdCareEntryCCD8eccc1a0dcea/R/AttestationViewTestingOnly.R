...
#' The XML element name is pcs-825abb87-f8f4-4776-b013-a17f5957f47e as a restriction of the DvFile
...
parseAttestationViewTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-825abb87-f8f4-4776-b013-a17f5957f47e', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvFile)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-825abb87-f8f4-4776-b013-a17f5957f47e'
      data$ccd <- XML::xmlName(root)
