...
#' \code{Attestation Reason - Testing Use Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Attestation Reason - Testing Use Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-308c49b5-1cf7-4178-bb55-2a8cd0a39051 as a restriction of the DvString
...
#' AttestationReasonTestingUseOnly <- getAttestationReasonTestingUseOnly(files) 
#' 
#' @export
getAttestationReasonTestingUseOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseAttestationReasonTestingUseOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseAttestationReasonTestingUseOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-308c49b5-1cf7-4178-bb55-2a8cd0a39051', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-308c49b5-1cf7-4178-bb55-2a8cd0a39051'
      data$ccd <- XML::xmlName(root)
