...
#' \code{Karnofsky Performance Status Score  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Karnofsky Performance Status Score  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-15d2fcfd-0d8f-430f-98ec-e53f6ec93a41 as a restriction of the DvOrdinal
...
#' @references
#' \code{rdfs:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?elementDetails=9&FirstTimer=0&PageId=ElementDetailsGroup&publicId=2003853&version=4.2} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' KarnofskyPerformanceStatusScoreTestingOnly <- getKarnofskyPerformanceStatusScoreTestingOnly(files) 
#' 
#' @export
getKarnofskyPerformanceStatusScoreTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseKarnofskyPerformanceStatusScoreTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseKarnofskyPerformanceStatusScoreTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-15d2fcfd-0d8f-430f-98ec-e53f6ec93a41', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvOrdinal)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-15d2fcfd-0d8f-430f-98ec-e53f6ec93a41'
      data$ccd <- XML::xmlName(root)
