...
#' The XML element name is pcs-9a9778a1-9675-4b3b-a230-57634955082f as a restriction of the DvString
...
parseParticipationModeTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-9a9778a1-9675-4b3b-a230-57634955082f', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-9a9778a1-9675-4b3b-a230-57634955082f'
      data$ccd <- XML::xmlName(root)
