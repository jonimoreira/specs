...
#' \code{Outras Neoplasias.}
#'
#' Returns a data.frame of the collected nodes of: \code{Outras Neoplasias} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-607c6f29-49ce-4dee-8a8f-51401144dd4e as a restriction of the DvString
...
#' @references
#' \code{rdfs:isDefinedBy} \url{ftp://ftp.datasus.gov.br/siscam/siscolo/Manual_SisColo4_Coordenacao.zip} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' OutrasNeoplasias <- getOutrasNeoplasias(files) 
#' 
#' @export
getOutrasNeoplasias <- function(sourceList)
{
    data <- lapply(sourceList, parseOutrasNeoplasias)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseOutrasNeoplasias <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-607c6f29-49ce-4dee-8a8f-51401144dd4e', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-607c6f29-49ce-4dee-8a8f-51401144dd4e'
      data$ccd <- XML::xmlName(root)
