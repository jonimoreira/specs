...
#' \code{Number of aiquots  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Number of aiquots  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-d34ecc85-250e-4b06-b328-d91695458756 as a restriction of the DvCount
...
#' 
#' Number of biospecimen aliquots.
#' 
#' @references
...
#' @references
#' \code{rdfs:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?elementDetails=9&FirstTimer=0&PageId=ElementDetailsGroup&publicId=3192457&version=1.0} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' NumberofaiquotsTestingOnly <- getNumberofaiquotsTestingOnly(files) 
#' 
#' @export
getNumberofaiquotsTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseNumberofaiquotsTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseNumberofaiquotsTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-d34ecc85-250e-4b06-b328-d91695458756', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvCount)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-d34ecc85-250e-4b06-b328-d91695458756'
      data$ccd <- XML::xmlName(root)
