...
#' \code{Body Temperature -  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Body Temperature -  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-15d8cae0-beb4-4b7a-a7a5-85407960191c as a restriction of the DvQuantity
...
#' @references
#' \code{rdfs:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?elementDetails=9&FirstTimer=0&PageId=ElementDetailsGroup&publicId=2644401&version=1.0} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' BodyTemperatureTestingOnly <- getBodyTemperatureTestingOnly(files) 
#' 
#' @export
getBodyTemperatureTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseBodyTemperatureTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseBodyTemperatureTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-15d8cae0-beb4-4b7a-a7a5-85407960191c', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvQuantity)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-15d8cae0-beb4-4b7a-a7a5-85407960191c'
      data$ccd <- XML::xmlName(root)
