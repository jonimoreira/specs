...
#' The XML element name is pcs-02eee140-964c-43b2-be7e-f13e9f56e5ed as a restriction of the DvString
...
parseParticipationFunctionTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-02eee140-964c-43b2-be7e-f13e9f56e5ed', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-02eee140-964c-43b2-be7e-f13e9f56e5ed'
      data$ccd <- XML::xmlName(root)
