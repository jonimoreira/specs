...
#' The XML element name is pcs-ced24000-62f1-442a-a5b5-3cc1c4d51077 as a restriction of the DvLink
...
parseWorkflowIDTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-ced24000-62f1-442a-a5b5-3cc1c4d51077', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvLink)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-ced24000-62f1-442a-a5b5-3cc1c4d51077'
      data$ccd <- XML::xmlName(root)
