...
#' The XML element name is pcs-e30b92ce-2a07-4429-8759-964cb349c026 as a restriction of the DvString
...
parseAdditionalAddressLinesUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-e30b92ce-2a07-4429-8759-964cb349c026', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-e30b92ce-2a07-4429-8759-964cb349c026'
      data$ccd <- XML::xmlName(root)
