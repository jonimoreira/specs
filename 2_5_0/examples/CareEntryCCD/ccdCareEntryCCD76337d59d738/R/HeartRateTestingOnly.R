...
#' The XML element name is pcs-dc439229-024f-4d0c-80fa-96121a94b200 as a restriction of the DvCount
...
parseHeartRateTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-dc439229-024f-4d0c-80fa-96121a94b200', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvCount)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-dc439229-024f-4d0c-80fa-96121a94b200'
      data$ccd <- XML::xmlName(root)
