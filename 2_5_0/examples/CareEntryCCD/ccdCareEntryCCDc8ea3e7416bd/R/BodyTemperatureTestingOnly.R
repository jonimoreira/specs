...
#' The XML element name is pcs-74dbf015-3e2e-4db3-bc5c-987da951be82 as a restriction of the DvQuantity
...
parseBodyTemperatureTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-74dbf015-3e2e-4db3-bc5c-987da951be82', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvQuantity)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-74dbf015-3e2e-4db3-bc5c-987da951be82'
      data$ccd <- XML::xmlName(root)
