...
#' The XML element name is pcs-270d0b1a-d9fb-47e3-a766-da8fde1cbb3b as a restriction of the DvString
...
parseStateAbbreviationUSPSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-270d0b1a-d9fb-47e3-a766-da8fde1cbb3b', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-270d0b1a-d9fb-47e3-a766-da8fde1cbb3b'
      data$ccd <- XML::xmlName(root)
