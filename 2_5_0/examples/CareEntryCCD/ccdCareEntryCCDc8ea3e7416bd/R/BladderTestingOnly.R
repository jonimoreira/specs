...
#' \code{Bladder  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Bladder  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-8f2109ea-cc18-4feb-a0aa-ea53cf19bc81 as a restriction of the DvBoolean
...
#' @references
#' \code{rdfs:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?elementDetails=9&FirstTimer=0&PageId=ElementDetailsGroup&publicId=2182237&version=1.0} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' BladderTestingOnly <- getBladderTestingOnly(files) 
#' 
#' @export
getBladderTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseBladderTestingOnly)
...
parseBladderTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-8f2109ea-cc18-4feb-a0aa-ea53cf19bc81', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvBoolean)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-8f2109ea-cc18-4feb-a0aa-ea53cf19bc81'
      data$ccd <- XML::xmlName(root)
