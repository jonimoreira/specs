...
#' The XML element name is pcs-055a71b5-1a18-47fb-a096-35917e9a06bb as a restriction of the DvRatio
...
parseTestRatioTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-055a71b5-1a18-47fb-a096-35917e9a06bb', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvRatio)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-055a71b5-1a18-47fb-a096-35917e9a06bb'
      data$ccd <- XML::xmlName(root)
