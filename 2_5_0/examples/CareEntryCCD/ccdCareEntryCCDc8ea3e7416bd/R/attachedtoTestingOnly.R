...
#' \code{attached to - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{attached to - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-bffee76c-728f-4a94-8678-85b0ce43403d as a restriction of the DvLink
...
#' @references
#' \code{rdfs:isDefinedBy} \url{http://purl.obolibrary.org/obo/RO_0002371} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' attachedtoTestingOnly <- getattachedtoTestingOnly(files) 
#' 
#' @export
getattachedtoTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseattachedtoTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseattachedtoTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-bffee76c-728f-4a94-8678-85b0ce43403d', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvLink)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-bffee76c-728f-4a94-8678-85b0ce43403d'
      data$ccd <- XML::xmlName(root)
