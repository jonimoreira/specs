...
#' \code{Attestation View  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Attestation View  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-488b3ac4-fab7-4a06-ba6f-a1f3dd05bace as a restriction of the DvFile
#' The vectors are: label, vtb, vte, #' size, encoding, dvfile_language, media_type, compression_type, hash_result, hash_function, alt_txt, uri, media_content, text_content, #' ccd, sourceName,
#' 
...
#' AttestationViewTestingOnly <- getAttestationViewTestingOnly(files) 
#' 
#' @export
getAttestationViewTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseAttestationViewTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseAttestationViewTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-488b3ac4-fab7-4a06-ba6f-a1f3dd05bace', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvFile)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-488b3ac4-fab7-4a06-ba6f-a1f3dd05bace'
      data$ccd <- XML::xmlName(root)
