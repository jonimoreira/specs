...
#' The XML element name is pcs-a45d261f-7c96-4bbe-8ba8-2d565508dc88 as a restriction of the DvBoolean
...
parseFeverduration3weeksorlessTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-a45d261f-7c96-4bbe-8ba8-2d565508dc88', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvBoolean)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-a45d261f-7c96-4bbe-8ba8-2d565508dc88'
      data$ccd <- XML::xmlName(root)
