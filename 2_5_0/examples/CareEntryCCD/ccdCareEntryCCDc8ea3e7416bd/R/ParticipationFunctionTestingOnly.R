...
#' \code{Participation Function - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Participation Function - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-6e7aa037-bde1-4f56-a1d6-29102fc01278 as a restriction of the DvString
...
#' @references
#' \code{rdfs:isDefinedBy} \url{http://xmlns.com/foaf/spec/#glance} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' ParticipationFunctionTestingOnly <- getParticipationFunctionTestingOnly(files) 
#' 
#' @export
getParticipationFunctionTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseParticipationFunctionTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseParticipationFunctionTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-6e7aa037-bde1-4f56-a1d6-29102fc01278', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-6e7aa037-bde1-4f56-a1d6-29102fc01278'
      data$ccd <- XML::xmlName(root)
