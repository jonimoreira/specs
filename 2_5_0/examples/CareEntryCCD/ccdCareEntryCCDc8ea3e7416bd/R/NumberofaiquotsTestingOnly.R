...
#' The XML element name is pcs-bcd915d2-1bf6-4c25-8377-91157e43db40 as a restriction of the DvCount
...
parseNumberofaiquotsTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-bcd915d2-1bf6-4c25-8377-91157e43db40', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvCount)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-bcd915d2-1bf6-4c25-8377-91157e43db40'
      data$ccd <- XML::xmlName(root)
