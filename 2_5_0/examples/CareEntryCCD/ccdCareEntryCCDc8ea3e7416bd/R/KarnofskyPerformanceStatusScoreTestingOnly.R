...
#' The XML element name is pcs-701fbe6d-3658-4fe3-ba63-16a39cb40f86 as a restriction of the DvOrdinal
...
parseKarnofskyPerformanceStatusScoreTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-701fbe6d-3658-4fe3-ba63-16a39cb40f86', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvOrdinal)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-701fbe6d-3658-4fe3-ba63-16a39cb40f86'
      data$ccd <- XML::xmlName(root)
