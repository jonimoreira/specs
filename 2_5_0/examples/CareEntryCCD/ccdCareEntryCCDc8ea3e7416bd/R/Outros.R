...
#' The XML element name is pcs-e3896fb6-cf29-4ac9-a7bb-733917a23913 as a restriction of the DvString
...
parseOutros <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-e3896fb6-cf29-4ac9-a7bb-733917a23913', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-e3896fb6-cf29-4ac9-a7bb-733917a23913'
      data$ccd <- XML::xmlName(root)
