...
#' The XML element name is pcs-c9ffeb03-867f-42ec-8fe4-592bb8f03111 as a restriction of the DvBoolean
...
parseBladderTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-c9ffeb03-867f-42ec-8fe4-592bb8f03111', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvBoolean)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-c9ffeb03-867f-42ec-8fe4-592bb8f03111'
      data$ccd <- XML::xmlName(root)
