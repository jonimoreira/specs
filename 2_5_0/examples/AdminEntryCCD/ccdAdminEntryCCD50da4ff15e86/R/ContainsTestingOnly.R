...
#' \code{Contains  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Contains  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-422f37ef-3c71-41ea-a0a2-cd145bd25e34 as a restriction of the DvLink
...
#' @references
#' \code{rdfs:isDefinedBy} \url{http://purl.obolibrary.org/obo/RO_0001019} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' ContainsTestingOnly <- getContainsTestingOnly(files) 
#' 
#' @export
getContainsTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseContainsTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseContainsTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-422f37ef-3c71-41ea-a0a2-cd145bd25e34', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvLink)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-422f37ef-3c71-41ea-a0a2-cd145bd25e34'
      data$ccd <- XML::xmlName(root)
