...
#' The XML element name is pcs-2d7a26bc-9ad0-46a1-817c-6005cd335db0 as a restriction of the DvString
...
parseCityNameUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-2d7a26bc-9ad0-46a1-817c-6005cd335db0', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-2d7a26bc-9ad0-46a1-817c-6005cd335db0'
      data$ccd <- XML::xmlName(root)
