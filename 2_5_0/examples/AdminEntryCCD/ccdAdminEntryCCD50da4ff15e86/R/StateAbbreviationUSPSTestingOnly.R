...
#' The XML element name is pcs-cb869c75-dfbe-46c4-b355-3fe2369a1adb as a restriction of the DvString
...
parseStateAbbreviationUSPSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-cb869c75-dfbe-46c4-b355-3fe2369a1adb', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-cb869c75-dfbe-46c4-b355-3fe2369a1adb'
      data$ccd <- XML::xmlName(root)
