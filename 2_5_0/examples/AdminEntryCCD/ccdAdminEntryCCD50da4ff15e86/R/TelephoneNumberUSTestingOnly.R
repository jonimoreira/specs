...
#' \code{Telephone Number (US)  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Telephone Number (US)  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-be4fce18-2fab-4716-a956-5fb0d857b0a7 as a restriction of the DvString
...
#' TelephoneNumberUSTestingOnly <- getTelephoneNumberUSTestingOnly(files) 
#' 
#' @export
getTelephoneNumberUSTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseTelephoneNumberUSTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseTelephoneNumberUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-be4fce18-2fab-4716-a956-5fb0d857b0a7', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-be4fce18-2fab-4716-a956-5fb0d857b0a7'
      data$ccd <- XML::xmlName(root)
