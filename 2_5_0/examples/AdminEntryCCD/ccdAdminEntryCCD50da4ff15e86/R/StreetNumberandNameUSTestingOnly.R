...
#' The XML element name is pcs-4616d2bf-9ba2-4293-aed3-ab97eec933f9 as a restriction of the DvString
...
parseStreetNumberandNameUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-4616d2bf-9ba2-4293-aed3-ab97eec933f9', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-4616d2bf-9ba2-4293-aed3-ab97eec933f9'
      data$ccd <- XML::xmlName(root)
