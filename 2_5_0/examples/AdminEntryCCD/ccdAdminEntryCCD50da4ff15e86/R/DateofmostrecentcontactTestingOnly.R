...
#' The XML element name is pcs-39705d7f-3077-4ae7-9d33-3f170a809450 as a restriction of the DvTemporal
...
#' @references
#' \code{rdfs:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?listValidValuesForDataElements=9&tabClicked=2&PageId=GetDetailsGroup} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
parseDateofmostrecentcontactTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-39705d7f-3077-4ae7-9d33-3f170a809450', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvTemporal)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-39705d7f-3077-4ae7-9d33-3f170a809450'
      data$ccd <- XML::xmlName(root)
