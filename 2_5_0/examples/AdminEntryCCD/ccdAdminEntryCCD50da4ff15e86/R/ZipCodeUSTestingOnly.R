...
#' The XML element name is pcs-9280457b-eff5-491a-9137-f3899c33a053 as a restriction of the DvString
...
#' @references
#' \code{rdfs:isDefinedBy} \url{https://www.usps.com/} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
parseZipCodeUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-9280457b-eff5-491a-9137-f3899c33a053', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-9280457b-eff5-491a-9137-f3899c33a053'
      data$ccd <- XML::xmlName(root)
