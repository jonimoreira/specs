...
#' \code{Telephone Number North American Plan  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Telephone Number North American Plan  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-369de379-d09c-43aa-8334-8de02590f702 as a restriction of the DvString
...
#' TelephoneNumberNorthAmericanPlanTestingOnly <- getTelephoneNumberNorthAmericanPlanTestingOnly(files) 
#' 
#' @export
getTelephoneNumberNorthAmericanPlanTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseTelephoneNumberNorthAmericanPlanTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseTelephoneNumberNorthAmericanPlanTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-369de379-d09c-43aa-8334-8de02590f702', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-369de379-d09c-43aa-8334-8de02590f702'
      data$ccd <- XML::xmlName(root)
