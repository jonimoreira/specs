...
#' The XML element name is pcs-66087287-b89b-40fe-9710-e1af92062af6 as a restriction of the DvCount
...
parseHeartRateTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-66087287-b89b-40fe-9710-e1af92062af6', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvCount)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-66087287-b89b-40fe-9710-e1af92062af6'
      data$ccd <- XML::xmlName(root)
