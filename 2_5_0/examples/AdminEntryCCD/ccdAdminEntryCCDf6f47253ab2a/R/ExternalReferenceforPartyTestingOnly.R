...
#' \code{External Reference for Party  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{External Reference for Party  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-0672fe6a-943f-4297-8d59-8239b02c4fe1 as a restriction of the DvLink
...
#' ExternalReferenceforPartyTestingOnly <- getExternalReferenceforPartyTestingOnly(files) 
#' 
#' @export
getExternalReferenceforPartyTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseExternalReferenceforPartyTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseExternalReferenceforPartyTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-0672fe6a-943f-4297-8d59-8239b02c4fe1', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvLink)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-0672fe6a-943f-4297-8d59-8239b02c4fe1'
      data$ccd <- XML::xmlName(root)
