...
#' \code{MedicationAdministration dosage rate  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{MedicationAdministration dosage rate  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-ab34fd84-b21c-403b-9bc3-c6a4e0ed3fcd as a restriction of the DvRatio
...
#' MedicationAdministrationdosagerateTestingOnly <- getMedicationAdministrationdosagerateTestingOnly(files) 
#' 
#' @export
getMedicationAdministrationdosagerateTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseMedicationAdministrationdosagerateTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseMedicationAdministrationdosagerateTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-ab34fd84-b21c-403b-9bc3-c6a4e0ed3fcd', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvRatio)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-ab34fd84-b21c-403b-9bc3-c6a4e0ed3fcd'
      data$ccd <- XML::xmlName(root)
