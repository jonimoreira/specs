...
#' The XML element name is pcs-d5a8042c-e131-48f8-894c-8bf26592f393 as a restriction of the DvString
...
parseAdditionalAddressLinesUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-d5a8042c-e131-48f8-894c-8bf26592f393', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-d5a8042c-e131-48f8-894c-8bf26592f393'
      data$ccd <- XML::xmlName(root)
