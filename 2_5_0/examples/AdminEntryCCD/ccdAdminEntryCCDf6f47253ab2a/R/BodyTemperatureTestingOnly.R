...
#' The XML element name is pcs-7cb2b570-102f-4f0b-8d59-89739713c6f0 as a restriction of the DvQuantity
...
parseBodyTemperatureTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-7cb2b570-102f-4f0b-8d59-89739713c6f0', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvQuantity)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-7cb2b570-102f-4f0b-8d59-89739713c6f0'
      data$ccd <- XML::xmlName(root)
