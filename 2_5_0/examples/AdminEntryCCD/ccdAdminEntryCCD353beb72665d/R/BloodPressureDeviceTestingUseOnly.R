...
#' \code{Blood Pressure Device - Testing Use Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Blood Pressure Device - Testing Use Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-5e2f57e9-312f-415c-8a4e-64ce8af9ecba as a restriction of the DvString
...
#' @references
#' \code{rdfs:isDefinedBy} \url{http://www.mlhim.org/test_pcts} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
#' BloodPressureDeviceTestingUseOnly <- getBloodPressureDeviceTestingUseOnly(files) 
#' 
#' @export
getBloodPressureDeviceTestingUseOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseBloodPressureDeviceTestingUseOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseBloodPressureDeviceTestingUseOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-5e2f57e9-312f-415c-8a4e-64ce8af9ecba', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-5e2f57e9-312f-415c-8a4e-64ce8af9ecba'
      data$ccd <- XML::xmlName(root)
