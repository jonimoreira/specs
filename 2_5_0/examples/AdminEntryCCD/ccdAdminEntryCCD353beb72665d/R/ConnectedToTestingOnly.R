...
#' The XML element name is pcs-063c6c51-581f-4dc8-92d2-4bd0c808c894 as a restriction of the DvLink
...
#' @references
#' \code{rdfs:isDefinedBy} \url{http://purl.obolibrary.org/obo/RO_0002150} and can be accessed via the CCD. A CCD is an XML Schema with embeded RDF.
#' 
...
parseConnectedToTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-063c6c51-581f-4dc8-92d2-4bd0c808c894', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvLink)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-063c6c51-581f-4dc8-92d2-4bd0c808c894'
      data$ccd <- XML::xmlName(root)
