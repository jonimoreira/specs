...
#' The XML element name is pcs-f55c876b-2dd3-4a6f-a1a9-6f82836e3b7c as a restriction of the DvString
...
parseTelephoneNumberUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-f55c876b-2dd3-4a6f-a1a9-6f82836e3b7c', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-f55c876b-2dd3-4a6f-a1a9-6f82836e3b7c'
      data$ccd <- XML::xmlName(root)
