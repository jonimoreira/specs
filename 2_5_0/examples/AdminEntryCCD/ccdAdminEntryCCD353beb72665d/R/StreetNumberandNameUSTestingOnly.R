...
#' \code{Street Number and Name (US)  - Testing Only.}
#'
#' Returns a data.frame of the collected nodes of: \code{Street Number and Name (US)  - Testing Only} from the XML instance passed as fileName.
#' The source name and the CCD ID are also included in each row to assist in identifying the source of the data.
#' The XML element name is pcs-20ec33cd-493c-44ea-9262-623faa06f2fb as a restriction of the DvString
...
#' StreetNumberandNameUSTestingOnly <- getStreetNumberandNameUSTestingOnly(files) 
#' 
#' @export
getStreetNumberandNameUSTestingOnly <- function(sourceList)
{
    data <- lapply(sourceList, parseStreetNumberandNameUSTestingOnly)
    data <- data.table::rbindlist(data, fill=TRUE)
    return(data)
}

#' @export
parseStreetNumberandNameUSTestingOnly <- function(sourceName) {
  ...
  pcm <- XML::getNodeSet(root, '//mlhim2:pcs-20ec33cd-493c-44ea-9262-623faa06f2fb', nsDEF)
  data <- lapply(pcm, mlhim250rm::DvString)
  data <- data.table::rbindlist(data, fill=TRUE)
  if (length(data) > 0) { 
      data$ccd <- XML::xmlName(root)
      data$pcs <- 'pcs-20ec33cd-493c-44ea-9262-623faa06f2fb'
      data$ccd <- XML::xmlName(root)
