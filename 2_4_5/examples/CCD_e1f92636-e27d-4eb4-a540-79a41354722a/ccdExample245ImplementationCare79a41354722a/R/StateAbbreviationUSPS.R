...
#'
#' Returns a data.frame of the collected nodes of: \code{State Abbreviation (USPS)} from the XML instance passed as fileName.
#' The XML element name is el-aea44d76-771e-4c34-ae69-6d37643e96b7 as a restriction of the mlhim244:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' Used in US address cluster for two character US state abbreviation.
#' 
#' @references
...
#' StateAbbreviationUSPS <- getStateAbbreviationUSPS(files) 
#' 
#' @export
getStateAbbreviationUSPS <- function(fileList)
{
    ldata <- lapply(fileList, parseStateAbbreviationUSPS)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseStateAbbreviationUSPS <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-aea44d76-771e-4c34-ae69-6d37643e96b7/language', nsDEF)
      if (length(n) > 0)
      {
