...
#'
#' Returns a data.frame of the collected nodes of: \code{Height} from the XML instance passed as fileName.
#' The XML element name is el-1145a60e-8fad-46aa-878d-b41d9925ea4b as a restriction of the mlhim245:DvQuantity
...
#' @references
#' \code{rdf:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?elementDetails=9&FirstTimer=0&PageId=ElementDetailsGroup&publicId=649&version=4.1}
#' 
...
#' Height <- getHeight(files) 
#' 
#' @export
getHeight <- function(fileList)
{
    ldata <- lapply(fileList, parseHeight)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseHeight <- function(fileName)
{
  ...
  root <- xmlRoot(doc)
  pct <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim245:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/normal-status', nsDEF)
      if (length(n) > 0)
      {
        normal_status <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/magnitude', nsDEF)
      if (length(n) > 0)
       {
         magnitude <- as.numeric(xmlValue(n[[1]]))
       }
       n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/mlhim245:magnitude-status', nsDEF)
       if (length(n) > 0)
        {
         magnitude_status <- xmlValue(n[[1]])
        }
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/error', nsDEF)
      if (length(n) > 0)
       {
         error <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/accuracy', nsDEF)
      if (length(n) > 0)
       {
         accuracy <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-1145a60e-8fad-46aa-878d-b41d9925ea4b/ccd:el-9158aae9-4493-4d25-b3a9-d4608bbc9995/DvString-dv', nsDEF)
      if (length(n) > 0)
       {
