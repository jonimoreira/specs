...
#'
#' Returns a data.frame of the collected nodes of: \code{Body Weight UOM} from the XML instance passed as fileName.
#' The XML element name is el-2c13f479-4c71-4fe2-8fbb-82510a42f121 as a restriction of the mlhim245:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' Units of measure for body weight.
#' 
#' @references
...
#' BodyWeightUOM <- getBodyWeightUOM(files) 
#' 
#' @export
getBodyWeightUOM <- function(fileList)
{
    ldata <- lapply(fileList, parseBodyWeightUOM)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseBodyWeightUOM <- function(fileName)
{
  ...
  root <- xmlRoot(doc)
  pct <- getNodeSet(root, '//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim245:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/language', nsDEF)
      if (length(n) > 0)
      {
