...
#'
#' Returns a data.frame of the collected nodes of: \code{Attestation Reason - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc as a restriction of the mlhim244:DvCodedString
...
#' 
#' A finalized release of some information entity.
#' 
#' @references
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://usefulinc.com/ns/doap%23release}
#' 
...
#' AttestationReasonTest245 <- getAttestationReasonTest245(files) 
#' 
#' @export
getAttestationReasonTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseAttestationReasonTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseAttestationReasonTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/language', nsDEF)
      if (length(n) > 0)
      {
        language <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/terminology-abbrev', nsDEF)
      if (length(n) > 0)
      {
        terminology_abbrev <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/terminology-name', nsDEF)
      if (length(n) > 0)
      {
        terminology_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/terminology-version', nsDEF)
      if (length(n) > 0)
      {
        terminology_version <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-93d066ef-c6ed-4fec-b747-aa87c71bbbbc/terminology-code', nsDEF)
      if (length(n) > 0)
      {
