...
#'
#' Returns a data.frame of the collected nodes of: \code{beats per minute} from the XML instance passed as fileName.
#' The XML element name is el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5 as a restriction of the mlhim245:DvString
...
#' beatsperminute <- getbeatsperminute(files) 
#' 
#' @export
getbeatsperminute <- function(fileList)
{
    ldata <- lapply(fileList, parsebeatsperminute)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parsebeatsperminute <- function(fileName)
{
  ...
  root <- xmlRoot(doc)
  pct <- getNodeSet(root, '//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim245:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4b2d3e99-c031-48ba-a1b3-39793d32b2c5/language', nsDEF)
      if (length(n) > 0)
      {
