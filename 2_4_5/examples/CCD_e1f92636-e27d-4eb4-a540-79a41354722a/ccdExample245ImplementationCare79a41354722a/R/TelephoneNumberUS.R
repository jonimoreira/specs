...
#'
#' Returns a data.frame of the collected nodes of: \code{Telephone Number (US)} from the XML instance passed as fileName.
#' The XML element name is el-9c65478e-5cdf-413b-aa54-ce0574e48374 as a restriction of the mlhim244:DvString
...
#' TelephoneNumberUS <- getTelephoneNumberUS(files) 
#' 
#' @export
getTelephoneNumberUS <- function(fileList)
{
    ldata <- lapply(fileList, parseTelephoneNumberUS)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseTelephoneNumberUS <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-9c65478e-5cdf-413b-aa54-ce0574e48374/language', nsDEF)
      if (length(n) > 0)
      {
