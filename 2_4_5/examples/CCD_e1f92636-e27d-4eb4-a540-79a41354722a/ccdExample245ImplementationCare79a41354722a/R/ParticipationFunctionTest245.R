...
#'
#' Returns a data.frame of the collected nodes of: \code{Participation Function - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-cf7bb5ef-b6f0-42df-b144-aa475de9f010 as a restriction of the mlhim244:DvCodedString
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://xmlns.com/foaf/spec/#glance}
#' 
...
#' ParticipationFunctionTest245 <- getParticipationFunctionTest245(files) 
#' 
#' @export
getParticipationFunctionTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseParticipationFunctionTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseParticipationFunctionTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/language', nsDEF)
      if (length(n) > 0)
      {
        language <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/terminology-abbrev', nsDEF)
      if (length(n) > 0)
      {
        terminology_abbrev <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/terminology-name', nsDEF)
      if (length(n) > 0)
      {
        terminology_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/terminology-version', nsDEF)
      if (length(n) > 0)
      {
        terminology_version <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-cf7bb5ef-b6f0-42df-b144-aa475de9f010/terminology-code', nsDEF)
      if (length(n) > 0)
      {
