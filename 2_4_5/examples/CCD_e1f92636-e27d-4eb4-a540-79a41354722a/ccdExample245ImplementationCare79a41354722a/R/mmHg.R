...
#'
#' Returns a data.frame of the collected nodes of: \code{mmHg} from the XML instance passed as fileName.
#' The XML element name is el-1667f08f-9fed-461f-bb69-b206a61938e6 as a restriction of the mlhim245:DvString
...
#' 
#' @export
getmmHg <- function(fileList)
{
    ldata <- lapply(fileList, parsemmHg)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parsemmHg <- function(fileName)
{
  ...
  root <- xmlRoot(doc)
  pct <- getNodeSet(root, '//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim245:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1667f08f-9fed-461f-bb69-b206a61938e6/language', nsDEF)
      if (length(n) > 0)
      {
