...
#'
#' Returns a data.frame of the collected nodes of: \code{Body Temperature UOM} from the XML instance passed as fileName.
#' The XML element name is el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20 as a restriction of the mlhim245:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' Units of measure for body temperature.
#' 
#' @references
...
#' BodyTemperatureUOM <- getBodyTemperatureUOM(files) 
#' 
#' @export
getBodyTemperatureUOM <- function(fileList)
{
    ldata <- lapply(fileList, parseBodyTemperatureUOM)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseBodyTemperatureUOM <- function(fileName)
{
  ...
  root <- xmlRoot(doc)
  pct <- getNodeSet(root, '//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim245:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-9ac654c7-7f6c-42c8-8fea-d6ed7f167b20/language', nsDEF)
      if (length(n) > 0)
      {
