...
#'
#' Returns a data.frame of the collected nodes of: \code{System ID - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc as a restriction of the mlhim244:DvIdentifier
...
#' 
#' A reusable system ID for FeederAuditDetails.
#' 
#' @references
...
#' SystemIDTest245 <- getSystemIDTest245(files) 
#' 
#' @export
getSystemIDTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseSystemIDTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseSystemIDTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/language', nsDEF)
      if (length(n) > 0)
      {
        language <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/id-name', nsDEF)
      if (length(n) > 0)
      {
        id_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/issuer', nsDEF)
      if (length(n) > 0)
      {
        issuer <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-6f8999dc-5b17-4f9b-9a5f-55ec89848edc/assignor', nsDEF)
      if (length(n) > 0)
      {
