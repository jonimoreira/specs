...
#'
#' Returns a data.frame of the collected nodes of: \code{attached to - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4 as a restriction of the mlhim244:DvURI
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://purl.obolibrary.org/obo/RO_0002371}
#' 
...
#' attachedtoTest245 <- getattachedtoTest245(files) 
#' 
#' @export
getattachedtoTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseattachedtoTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseattachedtoTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4/DvURI-dv', nsDEF)
      if (length(n) > 0)
      {
        DvURI_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-3a54417d-d1d6-4294-b868-e7a9ab28f8c4/relation', nsDEF)
      if (length(n) > 0)
      {
