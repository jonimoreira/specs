...
#'
#' Returns a data.frame of the collected nodes of: \code{MedicationAdministration dosage rate - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-aea9a220-7ed5-444c-9044-5f26c584eed1 as a restriction of the mlhim244:DvRatio
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://www.hl7.org/implement/standards/fhir/medicationadministration-definitions.html#MedicationAdministration.dosage.rate}
#' 
...
#' MedicationAdministrationdosagerateTest245 <- getMedicationAdministrationdosagerateTest245(files) 
#' 
#' @export
getMedicationAdministrationdosagerateTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseMedicationAdministrationdosagerateTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseMedicationAdministrationdosagerateTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/normal-status', nsDEF)
      if (length(n) > 0)
      {
        normal_status <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/numerator', nsDEF)
      if (length(n) > 0)
       {
         numerator <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/denominator', nsDEF)
      if (length(n) > 0)
       {
         denominator <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/magnitude', nsDEF)
      if (length(n) > 0)
       {
      ...
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/error', nsDEF)
      if (length(n) > 0)
       {
         error <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/accuracy', nsDEF)
      if (length(n) > 0)
       {
         accuracy <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/DvString-dv', nsDEF)
      if (length(n) > 0)
       {
         numerator_units <- xmlValue(n[[1]])
       }
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/DvString-dv', nsDEF)
      if (length(n) > 0)
       {
         denominator_units <- xmlValue(n[[1]])
       }
      n <- getNodeSet(root, '//ccd:el-aea9a220-7ed5-444c-9044-5f26c584eed1/ccd:el-70d4b917-9f7c-4ffb-bfcf-a603c4938c10/DvString-dv', nsDEF)
      if (length(n) > 0)
       {
