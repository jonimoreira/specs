...
#'
#' Returns a data.frame of the collected nodes of: \code{Weight} from the XML instance passed as fileName.
#' The XML element name is el-60bb270c-11e6-46c6-b7c8-f3321f68240e as a restriction of the mlhim245:DvQuantity
...
#' @references
#' \code{rdf:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?elementDetails=9&FirstTimer=0&PageId=ElementDetailsGroup&publicId=2179689&version=4.0}
#' 
...
#' Weight <- getWeight(files) 
#' 
#' @export
getWeight <- function(fileList)
{
    ldata <- lapply(fileList, parseWeight)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseWeight <- function(fileName)
{
  ...
  root <- xmlRoot(doc)
  pct <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim245:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/normal-status', nsDEF)
      if (length(n) > 0)
      {
        normal_status <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/magnitude', nsDEF)
      if (length(n) > 0)
       {
         magnitude <- as.numeric(xmlValue(n[[1]]))
       }
       n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/mlhim245:magnitude-status', nsDEF)
       if (length(n) > 0)
        {
         magnitude_status <- xmlValue(n[[1]])
        }
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/error', nsDEF)
      if (length(n) > 0)
       {
         error <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/accuracy', nsDEF)
      if (length(n) > 0)
       {
         accuracy <- as.numeric(xmlValue(n[[1]]))
       }
      n <- getNodeSet(root, '//ccd:el-60bb270c-11e6-46c6-b7c8-f3321f68240e/ccd:el-2c13f479-4c71-4fe2-8fbb-82510a42f121/DvString-dv', nsDEF)
      if (length(n) > 0)
       {
