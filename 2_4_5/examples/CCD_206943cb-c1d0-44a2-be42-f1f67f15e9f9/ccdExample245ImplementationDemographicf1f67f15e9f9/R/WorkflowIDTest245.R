...
#'
#' Returns a data.frame of the collected nodes of: \code{Workflow ID - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-3b46a988-3845-4660-9f3c-bc09b23ca7b7 as a restriction of the mlhim244:DvURI
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://purl.obolibrary.org/obo/IAO_0000178}
#' 
...
#' WorkflowIDTest245 <- getWorkflowIDTest245(files) 
#' 
#' @export
getWorkflowIDTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseWorkflowIDTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseWorkflowIDTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7/DvURI-dv', nsDEF)
      if (length(n) > 0)
      {
        DvURI_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-3b46a988-3845-4660-9f3c-bc09b23ca7b7/relation', nsDEF)
      if (length(n) > 0)
      {
