...
#'
#' Returns a data.frame of the collected nodes of: \code{Date of Death - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-e66dc054-d696-45cf-9d34-f4b315213903 as a restriction of the mlhim244:DvTemporal
...
#' @references
#' \code{rdf:isDefinedBy} \url{https://cdebrowser.nci.nih.gov/CDEBrowser/search?elementDetails=9&FirstTimer=0&PageId=ElementDetailsGroup&publicId=2004152&version=3.0}
#' 
...
#' DateofDeathTest245 <- getDateofDeathTest245(files) 
#' 
#' @export
getDateofDeathTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseDateofDeathTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseDateofDeathTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/normal-status', nsDEF)
      if (length(n) > 0)
      {
        normal_status <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-date', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_date <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-time', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_time <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-datetime', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_datetime <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-day', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_day <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-month', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_month <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-year', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_year <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-year-month', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_year_month <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-month-day', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_month_day <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-duration', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_duration <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-ymduration', nsDEF)
      if (length(n) > 0)
      {
        dvtemporal_ymduration <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-e66dc054-d696-45cf-9d34-f4b315213903/dvtemporal-dtduration', nsDEF)
      if (length(n) > 0)
      {
