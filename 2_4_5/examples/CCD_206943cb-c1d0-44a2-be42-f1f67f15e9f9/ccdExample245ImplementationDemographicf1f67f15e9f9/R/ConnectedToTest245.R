...
#'
#' Returns a data.frame of the collected nodes of: \code{Connected To - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-c0bfcffb-8891-4b60-a3a9-fafc716d4283 as a restriction of the mlhim244:DvURI
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://purl.obolibrary.org/obo/RO_0002150}
#' 
...
#' ConnectedToTest245 <- getConnectedToTest245(files) 
#' 
#' @export
getConnectedToTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseConnectedToTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseConnectedToTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283/DvURI-dv', nsDEF)
      if (length(n) > 0)
      {
        DvURI_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-c0bfcffb-8891-4b60-a3a9-fafc716d4283/relation', nsDEF)
      if (length(n) > 0)
      {
