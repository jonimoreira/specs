...
#'
#' Returns a data.frame of the collected nodes of: \code{Longitude (Decimal Format)} from the XML instance passed as fileName.
#' The XML element name is el-00fad715-e204-4505-8ff4-57e0c4bb3cc3 as a restriction of the mlhim244:DvString
...
#' LongitudeDecimalFormat <- getLongitudeDecimalFormat(files) 
#' 
#' @export
getLongitudeDecimalFormat <- function(fileList)
{
    ldata <- lapply(fileList, parseLongitudeDecimalFormat)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseLongitudeDecimalFormat <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-00fad715-e204-4505-8ff4-57e0c4bb3cc3/language', nsDEF)
      if (length(n) > 0)
      {
