...
#'
#' Returns a data.frame of the collected nodes of: \code{Condition severity - Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-4716f056-aa09-46bf-a6e5-5e79e298217b as a restriction of the mlhim244:DvCodedString
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://www.hl7.org/implement/standards/fhir/condition-definitions.html#Condition.severity}
#' 
...
#' ConditionseverityTest245 <- getConditionseverityTest245(files) 
#' 
#' @export
getConditionseverityTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseConditionseverityTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseConditionseverityTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/language', nsDEF)
      if (length(n) > 0)
      {
        language <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/terminology-abbrev', nsDEF)
      if (length(n) > 0)
      {
        terminology_abbrev <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/terminology-name', nsDEF)
      if (length(n) > 0)
      {
        terminology_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/terminology-version', nsDEF)
      if (length(n) > 0)
      {
        terminology_version <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4716f056-aa09-46bf-a6e5-5e79e298217b/terminology-code', nsDEF)
      if (length(n) > 0)
      {
