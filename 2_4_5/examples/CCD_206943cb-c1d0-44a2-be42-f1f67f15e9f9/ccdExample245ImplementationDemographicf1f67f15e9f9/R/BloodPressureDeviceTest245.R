...
#'
#' Returns a data.frame of the collected nodes of: \code{Blood Pressure Device (Test 2.4.5)} from the XML instance passed as fileName.
#' The XML element name is el-4e3b5bc7-1908-449c-90ab-72b23ac47462 as a restriction of the mlhim244:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' The device use in taking a blood pressure measurement.
#' 
#' @references
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://www.mlhim.org/test_pcts}
#' 
...
#' BloodPressureDeviceTest245 <- getBloodPressureDeviceTest245(files) 
#' 
#' @export
getBloodPressureDeviceTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parseBloodPressureDeviceTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseBloodPressureDeviceTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-4e3b5bc7-1908-449c-90ab-72b23ac47462/language', nsDEF)
      if (length(n) > 0)
      {
