...
#'
#' Returns a data.frame of the collected nodes of: \code{year(s)} from the XML instance passed as fileName.
#' The XML element name is el-a354bc1c-b417-472c-a50c-c503c982dd01 as a restriction of the mlhim244:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' Units in years for age
#' 
#' @references
...
#' 
#' @export
getyears <- function(fileList)
{
    ldata <- lapply(fileList, parseyears)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseyears <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-a354bc1c-b417-472c-a50c-c503c982dd01/language', nsDEF)
      if (length(n) > 0)
      {
