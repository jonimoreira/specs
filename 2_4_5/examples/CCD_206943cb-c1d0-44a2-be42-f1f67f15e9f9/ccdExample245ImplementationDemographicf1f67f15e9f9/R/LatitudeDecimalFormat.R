...
#'
#' Returns a data.frame of the collected nodes of: \code{Latitude (Decimal Format)} from the XML instance passed as fileName.
#' The XML element name is el-fdc67dd1-a796-40e6-b462-9b370ed3b323 as a restriction of the mlhim244:DvString
...
#' LatitudeDecimalFormat <- getLatitudeDecimalFormat(files) 
#' 
#' @export
getLatitudeDecimalFormat <- function(fileList)
{
    ldata <- lapply(fileList, parseLatitudeDecimalFormat)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseLatitudeDecimalFormat <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fdc67dd1-a796-40e6-b462-9b370ed3b323/language', nsDEF)
      if (length(n) > 0)
      {
