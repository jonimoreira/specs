...
#'
#' Returns a data.frame of the collected nodes of: \code{Street Number and Name (US)} from the XML instance passed as fileName.
#' The XML element name is el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8 as a restriction of the mlhim244:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' A single line US Street number and name. The typical format is:
#' 123 Main St. 
#' 
#' @references
...
#' StreetNumberandNameUS <- getStreetNumberandNameUS(files) 
#' 
#' @export
getStreetNumberandNameUS <- function(fileList)
{
    ldata <- lapply(fileList, parseStreetNumberandNameUS)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseStreetNumberandNameUS <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-ffa9835f-7ff3-4be8-ad9f-0fa536f8b7b8/language', nsDEF)
      if (length(n) > 0)
      {
