...
#'
#' Returns a data.frame of the collected nodes of: \code{bpm Test 2.4.5} from the XML instance passed as fileName.
#' The XML element name is el-fef4ff7b-148f-4318-bf22-2b715a2ac20b as a restriction of the mlhim244:DvString
...
#' bpmTest245 <- getbpmTest245(files) 
#' 
#' @export
getbpmTest245 <- function(fileList)
{
    ldata <- lapply(fileList, parsebpmTest245)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parsebpmTest245 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fef4ff7b-148f-4318-bf22-2b715a2ac20b/language', nsDEF)
      if (length(n) > 0)
      {
