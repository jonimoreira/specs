...
#'
#' Returns a data.frame of the collected nodes of: \code{Additional Address Line(s) (US)} from the XML instance passed as fileName.
#' The XML element name is el-d102a7b2-675c-432c-b7a2-1df1620e1ccc as a restriction of the mlhim244:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' Used for additional address lines in the US such as building name or apartment number.
#' 
#' @references
...
#' AdditionalAddressLinesUS <- getAdditionalAddressLinesUS(files) 
#' 
#' @export
getAdditionalAddressLinesUS <- function(fileList)
{
    ldata <- lapply(fileList, parseAdditionalAddressLinesUS)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseAdditionalAddressLinesUS <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-d102a7b2-675c-432c-b7a2-1df1620e1ccc/language', nsDEF)
      if (length(n) > 0)
      {
