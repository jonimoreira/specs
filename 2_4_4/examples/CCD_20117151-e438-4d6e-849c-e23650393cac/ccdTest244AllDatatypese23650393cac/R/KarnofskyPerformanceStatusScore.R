...
#'
#' Returns a data.frame of the collected nodes of: \code{Karnofsky Performance Status Score} from the XML instance passed as fileName.
#' The XML element name is el-fbee469e-174f-42a0-9e67-6d6db531352c as a restriction of the mlhim244:DvOrdinal
...
#' KarnofskyPerformanceStatusScore <- getKarnofskyPerformanceStatusScore(files) 
#' 
#' @export
getKarnofskyPerformanceStatusScore <- function(fileList)
{
    ldata <- lapply(fileList, parseKarnofskyPerformanceStatusScore)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseKarnofskyPerformanceStatusScore <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/normal-status', nsDEF)
      if (length(n) > 0)
      {
        normal_status <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/DvOrdinal-dv', nsDEF)
      if (length(n) > 0)
      {
        DvOrdinal_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-fbee469e-174f-42a0-9e67-6d6db531352c/symbol', nsDEF)
      if (length(n) > 0)
      {
        symbol <- xmlValue(n[[1]])
      }
