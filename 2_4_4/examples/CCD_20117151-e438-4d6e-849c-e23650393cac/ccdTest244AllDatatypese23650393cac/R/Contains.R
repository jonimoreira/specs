...
#'
#' Returns a data.frame of the collected nodes of: \code{Contains} from the XML instance passed as fileName.
#' The XML element name is el-da990c3e-c752-4787-9b3a-d6f4f4051f95 as a restriction of the mlhim244:DvURI
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvURI_dv, relation
#' 
#'  Intended meaning: domain: spatial region or site (immaterial continuant) range: material entity
#' 
#' @references
...
#' @references
#' \code{rdf:isDefinedBy} \url{http://purl.obolibrary.org/obo/RO_0001019}
#' 
...
#' Contains <- getContains(files) 
#' 
#' @export
getContains <- function(fileList)
{
    ldata <- lapply(fileList, parseContains)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseContains <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95/DvURI-dv', nsDEF)
      if (length(n) > 0)
      {
        DvURI_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-da990c3e-c752-4787-9b3a-d6f4f4051f95/relation', nsDEF)
      if (length(n) > 0)
      {
