...
#'
#' Returns a data.frame of the collected nodes of: \code{Test DvParsable for 2.4.4} from the XML instance passed as fileName.
#' The XML element name is el-1671670b-e1db-4d71-a7dd-6bb09c025902 as a restriction of the mlhim244:DvParsable
...
#' TestDvParsablefor244 <- getTestDvParsablefor244(files) 
#' 
#' @export
getTestDvParsablefor244 <- function(fileList)
{
    ldata <- lapply(fileList, parseTestDvParsablefor244)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseTestDvParsablefor244 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/size', nsDEF)
      if (length(n) > 0)
      {
        size <- as.numeric(xmlValue(n[[1]]))
      }
      
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/encoding', nsDEF)
      if (length(n) > 0)
      {
        encoding <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/language', nsDEF)
      if (length(n) > 0)
      {
        language <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/DvParsable-dv', nsDEF)
      if (length(n) > 0)
      {
        DvParsable_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-1671670b-e1db-4d71-a7dd-6bb09c025902/formalism', nsDEF)
      if (length(n) > 0)
      {
