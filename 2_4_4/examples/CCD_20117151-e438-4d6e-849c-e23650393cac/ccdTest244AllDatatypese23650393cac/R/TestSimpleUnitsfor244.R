...
#'
#' Returns a data.frame of the collected nodes of: \code{Test Simple Units for 2.4.4} from the XML instance passed as fileName.
#' The XML element name is el-d6ce7add-82bb-4051-a503-ea61ed3589aa as a restriction of the mlhim244:DvString
#' The vectors are: ccd, data_name, ev-name, valid_time_begin, valid_time_end, ev_name, fileName,
#' DvString_dv, language
#' 
#' Test Simple Units for 2.4.4
#' 
#' @references
...
#' TestSimpleUnitsfor244 <- getTestSimpleUnitsfor244(files) 
#' 
#' @export
getTestSimpleUnitsfor244 <- function(fileList)
{
    ldata <- lapply(fileList, parseTestSimpleUnitsfor244)
    clean <- ldata[!is.na(ldata)]
    data <- do.call(rbind,clean)
    return(data)
}

parseTestSimpleUnitsfor244 <- function(fileName)
{
  ...
  pct <- getNodeSet(root, '//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa', nsDEF)
  ...
  if (length(pct) > 0)
  {
      ...
      children <- getNodeSet(root, '//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa/child::node()', nsDEF)
      if (!is.na(charmatch('mlhim244:',children[[2]]))) # the exceptional value elements are in the RM namespace
      {
        n <- getNodeSet(root, paste('//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa/',xmlName(children[[2]]),'/ev-name'), nsDEF)
        ev_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa/data-name', nsDEF)
      if (length(n) > 0)
      {
        data_name <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa/valid-time-begin', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa/valid-time-end', nsDEF)
      if (length(n) > 0)
      {
        ds <- xmlValue(n[[1]])
        ds <- gsub('T', ' ', ds)
      ...
      n <- getNodeSet(root, '//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa/DvString-dv', nsDEF)
      if (length(n) > 0)
      {
        DvString_dv <- xmlValue(n[[1]])
      }
      
      n <- getNodeSet(root, '//ccd:el-d6ce7add-82bb-4051-a503-ea61ed3589aa/language', nsDEF)
      if (length(n) > 0)
      {
